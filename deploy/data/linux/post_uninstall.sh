...
APP_NAME=ВадькаVPNVPN
ORG_NAME=ВадькаVPNVPN.ORG
...
ls /opt/ВадькаVPNVPN/client/lib/* | while IFS=: read -r dir; do
...
if test -f "$LOG_FOLDER/ВадькаVPNVPN-service.log"; then
    sudo rm -f "$LOG_FOLDER/ВадькаVPNVPN-service.log" >> $LOG_FILE 2>&1
fi
