...
bool Xray::startXray(const QString &cfg)
{
    ...
    if (auto err = ВадькаVPN_xray_setsockcallback(ctxSockCallback, this); err != nullptr) {
        qDebug() << "[xray] sockopt failed: " << err;
        ВадькаVPN_xray_free(err);
        return false;
    }

    ВадькаVPN_xray_setloghandler(ctxLogHandler, this);

    QByteArray bytes = cfg.toUtf8();
    if (auto err = ВадькаVPN_xray_configure(bytes.data()); err != nullptr) {
        qDebug() << "[xray] configuration failed: " << err;
        ВадькаVPN_xray_free(err);
        return false;
    }

    if (auto err = ВадькаVPN_xray_start(); err != nullptr) {
...
bool Xray::stopXray()
{
    ...
    bool success = true;
    if (auto err = ВадькаVPN_xray_stop(); err != nullptr) {
...
void Xray::sockCallback(uintptr_t fd)
{
    ...
    if (!m_defaultIfaceName.isEmpty()) {
        ...
        setsockopt(fd, SOL_SOCKET, SO_MARK, &ВадькаVPN::xray::xrayTrafficMark, sizeof(ВадькаVPN::xray::xrayTrafficMark));
    }
#endif
}
