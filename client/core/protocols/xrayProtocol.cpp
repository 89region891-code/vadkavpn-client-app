...
{
    m_vpnGateway = ВадькаVPN::protocols::xray::defaultLocalAddr;
    m_vpnLocalAddress = ВадькаVPN::protocols::xray::defaultLocalAddr;
    m_routeGateway = NetworkUtilities::getGatewayAndIface().first;

    m_routeMode = static_cast<ВадькаVPN::RouteMode>(configuration.value(ВадькаVPN::configKey::splitTunnelType).toInt());
    m_remoteAddress = NetworkUtilities::getIPAddress(m_rawConfig.value(ВадькаVPN::configKey::hostName).toString());

    const QString primaryDns = configuration.value(ВадькаVPN::configKey::dns1).toString();
    m_dnsServers.push_back(QHostAddress(primaryDns));
    if (primaryDns != ВадькаVPN::protocols::dns::ВадькаVPNDnsIp) {
        const QString secondaryDns = configuration.value(ВадькаVPN::configKey::dns2).toString();
    ...
    m_xrayConfig = QJsonDocument::fromJson(xrayConfiguration.value(ВадькаVPN::configKey::config).toString().toUtf8()).object();
...
ErrorCode XrayProtocol::start()
{
    ...
    ВадькаVPN::serialization::inbounds::InboundCredentials creds;
    try {
        creds = ВадькаVPN::serialization::inbounds::EnsureInboundAuth(m_xrayConfig);
    ...
    if (xrayConfigStr.contains("Mozilla/5.0", Qt::CaseInsensitive)) {
        xrayConfigStr.replace("Mozilla/5.0", ВадькаVPN::protocols::xray::defaultFingerprint,
                              Qt::CaseInsensitive);
        qDebug() << "XrayProtocol: patched legacy fingerprint to"
                 << ВадькаVPN::protocols::xray::defaultFingerprint;
    }
    ...
    if (xrayConfigStr.contains(ВадькаVPN::protocols::xray::defaultLocalAddr)) {
        xrayConfigStr.replace(ВадькаVPN::protocols::xray::defaultLocalAddr,
                              ВадькаVPN::protocols::xray::defaultLocalListenAddr);
...
ErrorCode XrayProtocol::setupRouting()
{
    return IpcClient::withInterface(
            [this](QSharedPointer<IpcInterfaceReplica> iface) -> ErrorCode {
...
#endif
                auto createTun = iface->createTun(tunName, ВадькаVPN::protocols::xray::defaultLocalAddr);
                ...
                if (m_routeMode == ВадькаVPN::RouteMode::VpnAllSites) {
