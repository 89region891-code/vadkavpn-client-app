...
ErrorCode SshSession::runContainerScript(const ServerCredentials &credentials, DockerContainer container, QString script,
                                         ...
                                         const std::function<ErrorCode(const QString &, libssh::Client &)> &cbReadStdErr)
{
    QString fileName = "/opt/ВадькаVPN/" + Utils::getRandomString(16) + ".sh";
    ...
    e = runScript(credentials, replaceVars(runner, ВадькаVPN::genBaseVars(credentials, container, QString(), QString())), cbReadStdOut, cbReadStdErr);

    QString remover = QString("sudo docker exec -i $CONTAINER_NAME rm %1 ").arg(fileName);
    runScript(credentials, replaceVars(remover, ВадькаVPN::genBaseVars(credentials, container, QString(), QString())), cbReadStdOut, cbReadStdErr);
                                                ...
                                                const QString &path, libssh::ScpOverwriteMode overwriteMode)
{
    ...
    e = runScript(credentials, replaceVars(mkdir, ВадькаVPN::genBaseVars(credentials, container, QString(), QString())));
    if (e)
        return e;
    ...
    runScript(credentials, replaceVars(QString("sudo shred -u %1").arg(tmpFileName), ВадькаVPN::genBaseVars(credentials, container, QString(), QString())));
    return e;
}
...
QString SshSession::checkSshConnection(const ServerCredentials &credentials, ErrorCode &errorCode)
{
    ...
    errorCode = runScript(credentials, ВадькаVPN::scriptData(SharedScriptType::check_connection), cbReadStdOut, cbReadStdErr);
