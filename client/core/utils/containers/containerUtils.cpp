...
QString ContainerUtils::containerToString(DockerContainer c)
{
    ...
    if (c == DockerContainer::Cloak)
        return "ВадькаVPN-openvpn-cloak";
    if (c == DockerContainer::Awg)
        return "ВадькаVPN-awg";
    if (c == DockerContainer::Awg2)
        return "ВадькаVPN-awg2";
    QMetaEnum metaEnum = QMetaEnum::fromType<DockerContainer>();
    QString containerKey = metaEnum.valueToKey(static_cast<int>(c));

    return "ВадькаVPN-" + containerKey.toLower();
}
...
QMap<DockerContainer, QString> ContainerUtils::containerHumanNames()
{
    return { { DockerContainer::None, "Not installed" },
             ...
             { DockerContainer::Awg, "ВадькаVPNWG" },
             { DockerContainer::Awg2, "ВадькаVPNWG" },
             { DockerContainer::Xray, "XRay" },
             { DockerContainer::Ipsec, QObject::tr("IPsec") },
             { DockerContainer::SSXray, "Shadowsocks"},

             { DockerContainer::TorWebSite, QObject::tr("Website in Tor network") },
             { DockerContainer::Dns, QObject::tr("ВадькаVPNDNS") },
    ...
    return {              { DockerContainer::OpenVpn,
             ...
             { DockerContainer::WireGuard,
               QObject::tr("WireGuard - popular VPN protocol with high performance, high speed and low power "
                           "consumption.") },
             { DockerContainer::Awg,
               QObject::tr("ВадькаVPNWG is a special protocol from ВадькаVPN based on WireGuard. "
...
QMap<DockerContainer, QString> ContainerUtils::containerDetailedDescriptions()
{
    return {
        ...
        { DockerContainer::Awg2,
          QObject::tr("ВадькаVPNWG is a modern VPN protocol based on WireGuard, "
        ...
        { DockerContainer::Sftp,
          QObject::tr("After installation, ВадькаVPN will create a\n\n file storage on your server. "
...
{
    switch (container) {
    case DockerContainer::Awg2: return QObject::tr("ВадькаVPNWG protocol will be installed. "
                                         "It provides high connection speed and ensures stable operation even in the most challenging network conditions.");
    default: return "";
    }
}
