...
NSString* MacOSUtils::appId() {
  ...
  if (!appId) {
    ...
    appId = @"org.ВадькаVPN.ВадькаVPNVPN";
  }
  ...
  if (@available(macOS 13, *)) {
    ...
    if (startAtBoot) {
      if (![[SMAppService mainAppService] registerAndReturnError: & error]) {
        logger.error() << "Failed to register ВадькаVPN VPN LoginItem: " << error.localizedDescription;
      } else {
        logger.debug() << "ВадькаVPN VPN LoginItem registered successfully.";
      }
    } else {
      if (![[SMAppService mainAppService] unregisterAndReturnError: & error]) {
        logger.error() << "Failed to unregister ВадькаVPN VPN LoginItem: " << error.localizedDescription;
      } else {
