...
constexpr const char* TMP_PATH = "/tmp/ВадькаVPNvpn.socket";
constexpr const char* VAR_PATH = "/var/run/ВадькаVPNvpn/daemon.socket";
#endif
...
QString DaemonLocalServer::daemonPath() const {
  ...
  if (dir.exists("ВадькаVPNvpn")) {
    logger.debug() << "/var/run/ВадькаVPNvpn seems to be usable";
    return VAR_PATH;
  }

  if (!dir.mkdir("ВадькаVPNvpn")) {
    logger.warning() << "Failed to create /var/run/ВадькаVPNvpn";
    return TMP_PATH;
  }

  if (chmod("/var/run/ВадькаVPNvpn", S_IRWXU | S_IRWXG | S_IRWXO) < 0) {
    logger.warning()
        << "Failed to set the right permissions to /var/run/ВадькаVPNvpn";
    return TMP_PATH;
  }
