...
if [ "$CHECK_CONTAINERS" = "1" ] && [ "$USER_GOOD" != "0" ]; then
  ...
  ВадькаVPN_cont=$($SUD docker ps -a 2>/dev/null | grep -c ВадькаVPN || echo "0")

  log_and_display "Containers check: Total $total_cont / Active $active_cont / ВадькаVPN $ВадькаVPN_cont"
  $SUD docker ps -a --format "{{.Names}} ({{.Image}}) ({{.Status}}) ({{.Ports}})" 2>/dev/null | grep ВадькаVPN || true
  
  # Peers check
  if $SUD docker ps 2>/dev/null | grep -qE '\<(ВадькаVPN-awg|ВадькаVPN-wireguard)\>'; then
    log_and_display ""
    log_and_display "Peers check (beta):"
    if $SUD docker ps 2>/dev/null | grep -q ВадькаVPN-awg; then
      ВадькаVPN_WG_CONTAINER=$($SUD docker ps 2>/dev/null | grep ВадькаVPN-awg | awk '{print $1}' | head -1)
      if [ -n "$ВадькаVPN_WG_CONTAINER" ]; then
        WG_PEERS=$($SUD docker exec -it "$ВадькаVPN_WG_CONTAINER" wg show 2>/dev/null | grep -c 'peer' || echo "0")
        log_and_display "ВадькаVPNWG peers: $WG_PEERS"
      fi
    fi
    if $SUD docker ps 2>/dev/null | grep -q ВадькаVPN-wireguard; then
      WIREGUARD_CONTAINER=$($SUD docker ps 2>/dev/null | grep ВадькаVPN-wireguard | awk '{print $1}' | head -1)
