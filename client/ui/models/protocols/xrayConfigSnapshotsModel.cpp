...
XrayConfigSnapshot XrayConfigSnapshot::fromJson(const QJsonObject &json)
{
    ...
    s.serverConfig = ВадькаVPN::XrayServerConfig::fromJson(json.value("serverConfig").toObject());
    return s;
}
...
void XrayConfigSnapshotsModel::createFromCurrent(const ВадькаVPN::XrayServerConfig &serverConfig)
{
...
ВадькаVPN::XrayServerConfig XrayConfigSnapshotsModel::applyConfig(int index) const
{
    if (index < 0 || index >= m_configs.size()) {
        return ВадькаVPN::XrayServerConfig {};
    }
...
QString XrayConfigSnapshotsModel::buildDisplayName(const ВадькаVPN::XrayServerConfig &cfg)
{
...
void XrayConfigSnapshotsModel::applyConfigToCurrentModel(int index)
{
    ...
    ВадькаVPN::XrayServerConfig cfg = applyConfig(index);
    if (cfg.port.isEmpty()) {
