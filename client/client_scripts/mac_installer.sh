...
echo "[ВадькаVPNVPN] Installer package: $INSTALLER_PATH"

if [ ! -f "$INSTALLER_PATH" ]; then
    echo "[ВадькаVPNVPN] ERROR: Installer package not found: $INSTALLER_PATH"
    exit 1
fi

PKG_PATH="$INSTALLER_PATH"
echo "[ВадькаVPNVPN] Using PKG: $PKG_PATH"
...
echo "[ВадькаVPNVPN] Running installer..."
OSA_CMD='do shell script "/usr/sbin/installer -pkg '"$PKG_PATH"' -target /" with administrator privileges'
osascript -e "$OSA_CMD"

STATUS=$?
if [ $STATUS -ne 0 ]; then
    echo "[ВадькаVPNVPN] ERROR: installer exited with status $STATUS"
    exit $STATUS
fi

echo "[ВадькаVPNVPN] Cleaning up..."
rm -f "$INSTALLER_PATH" || true
rm -rf "$EXTRACT_DIR" 2>/dev/null || true

echo "[ВадькаVPNVPN] Installation completed successfully"
exit 0
